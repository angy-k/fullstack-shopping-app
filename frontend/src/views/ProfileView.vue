            ...
            <v-form @submit.prevent="updateProfile" ref="form">
              <v-row>
                ...
                <v-col cols="12">
                  ...
                  <v-card-subtitle class="px-0 mb-4">Leave blank if you don't want to change your password</v-card-subtitle>
                </v-col>
...
const isLoading = computed(() => {
  ...
  return authStore.loading || !authStore.user || (authStore.isAuthenticated && !authStore.user);
});
...
.skeleton-line {
  height: 20px;
  background: linear-gradient(90deg, rgba(var(--v-theme-on-surface), 0.1), rgba(var(--v-theme-on-surface), 0.05), rgba(var(--v-theme-on-surface), 0.1));
...
.skeleton-button {
  ...
  width: 120px;
  border-radius: 4px;
  background: linear-gradient(90deg, rgba(var(--v-theme-primary), 0.2), rgba(var(--v-theme-primary), 0.1), rgba(var(--v-theme-primary), 0.2));
  background-size: 200% 100%;
  animation: shimmer 1.5s infinite;
}
