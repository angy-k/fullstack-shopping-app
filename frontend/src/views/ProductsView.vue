<template>
  <v-container class="custom-container">
    ...
    <!-- Products Grid -->
    <v-row v-if="loading">
      <v-col v-for="i in 12" :key="`skeleton-${i}`" cols="12" sm="6" md="4" lg="3">
        <ProductCardSkeleton />
      </v-col>
    </v-row>
...
const fetchCategories = async () => {
  try {
    ...
    if (selectedCategoryId.value) {
      ...
      const selectedCategory = categories.value.find(cat => String(cat.id) === categoryId);
      
      if (selectedCategory) {
        console.log(`Found category: ${selectedCategory.name} for ID: ${categoryId}`);
...
const parseUrlParams = () => {
  ...
  if (categoryParam) {
    ...
    if (selectedCategory) {
      ...
      console.log(`Category ID ${categoryId} found in URL, waiting for categories to load`);
    }
  }
